struct LinearCumSum {
  int n, B;
  vector<ll> A;
  vector<vector<vector<ll>>> S, X;
  LinearCumSum(vector<ll> a) : n(a.size()), B((int)sqrt(n)), A(a), S(B+1), X(B+1) {
    for (int d = 1; d < B; ++d) {
      S[d].resize(d+1);
      X[d].resize(d+1);
      rep(k, d) {
        S[d][k].resize(n/d+2); X[d][k].resize(n/d+2);
      }
      rep(i, n) {
  ...
  ll sum(int l, int d, int k) { assert(l+d*(k-1) < n);
    ll s = 0;
    if (d >= B) { // naive
      int idx = 1;

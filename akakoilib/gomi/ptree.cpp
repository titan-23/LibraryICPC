...
struct Pool {
  ...
  static Node* lim = nullptr;
  static int idx = 0;
  static void expand() {
    if (idx == (int)blocks.size()) {
      Node* new_block = new Node[CHUNK_SIZE];
  ...
  static NP newnode(NP l, NP r, F lz, T v, U size, int dep, bool rev) {
    if (ptr == lim) expand();
...
NP newpar(NP l, NP r) {
  return Pool::newnode(l, r, id(), op(v(l), v(r)), size(l) + size(r), max(dep(l), dep(r)) + 1, false);
}
NP add(NP x, F alz) {
  if (!x) return nullptr;
  return Pool::newnode(x->l, x->r, composition(alz, x->lz), mapping(alz, x->v), x->size, x->dep, x->rev);
}
NP toggle(NP x) {
  if (!x) return nullptr;
  return Pool::newnode(x->r, x->l, x->lz, x->v, x->size, x->dep, !x->rev);
}
void push_rev(NP t, NP &l, NP &r) {
  if (t->rev) {
    l = toggle(t->l); r = toggle(t->r);
  } else {
    l = t->l; r = t->r;
  }
}
...
pair<NP, NP> merge1(NP a, NP b, NP c) {
...
pair<NP, NP> merge2(NP a, NP b, F alz, F blz) {
  if (dep(a) < dep(b)) {
    ...
    return merge1(x.first, x.second, add(br, blz));
  } else if (dep(a) > dep(b)) {
    alz = composition(alz, a->lz);
    NP al, ar; push_rev(a, al, ar);
    pair<NP, NP> x = merge2(ar, b, alz, blz);
    return merge1(add(al, alz), x.first, x.second);
...
vector<T> tovector(NP root) {
  vector<T> res;
  if (!root) return res;
  vector<tuple<NP, F, bool>> st;
  st.reserve(root->dep + 1);
  st.emplace_back(root, id(), false);
  while (!st.empty()) {
    auto [t, lz, flip] = st.back(); st.pop_back();
    if (t->size == 1) {
      res.push_back(mapping(lz, t->v));
    } else {
      F f = composition(lz, t->lz);
      if (t->r) st.emplace_back(t->r, f, flip^t->rev);
      if (t->l) st.emplace_back(t->l, f, flip^t->rev);
    }
  }
  return res;
}
struct Tree {
  ...
  Tree(vector<T> &v) {
    ...
    auto dfs = [&](auto &&dfs, int l, int r) -> NP {
      ...
      if (r - l == 1) return Pool::newnode( nullptr, nullptr, id(), v[l], 1, 0, false);
      int m = (l + r) >> 1;
      NP L = dfs(dfs, l, m);
      NP R = dfs(dfs, m, r);
      return newpar(L, R);
    };
    root = dfs(dfs, 0, (int)v.size());
  }
  ...
  T get(U k) { return prod(k, k + 1); }
  Tree insert(U k, T v) {
    auto [l, r] = split(root, k, id());
    return Tree(merge(merge(l, Pool::newnode(nullptr, nullptr, id(), v, 1, 0, false)), r));
  }
  Tree erase(U l, U r) {
  ...
  Tree apply(U l, U r, F f) {
    if (l >= r) return Tree(root);
    auto [abc, d] = split(root, r, id());
    auto [a, b] = split(abc, l, id());
    return Tree(merge(merge(a, add(b, f)), d));
  }
  Tree reverse(U l, U r) {
    if (l >= r) return Tree(root);
    auto [abc, d] = split(root, r, id());
    auto [a, b] = split(abc, l, id());
    return Tree(merge(merge(a, toggle(b)), d));
  }
...
void reserve(int n) {
  while (Pool::blocks.size()*CHUNK_SIZE < n) Pool::expand();
}

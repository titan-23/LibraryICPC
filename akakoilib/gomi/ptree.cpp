...
void push_rev(NP t, NP &l, NP &r) { // need `&`
  if (t->rev) { l = toggle(t->l); r = toggle(t->r); }
  else { l = t->l; r = t->r; }
}
pair<NP, NP> sub(NP a, NP b, NP c) {
  if (abs(max(dep(a), dep(b))+1-dep(c)) <= 1) return {gen(a, b), c};
  if (abs(max(dep(b), dep(c))+1-dep(a)) <= 1) return {a, gen(b, c)};
  NP bl, br; push_rev(b, bl, br);
  return {gen(a, push(bl, b->lz)), gen(push(br, b->lz), c)};
}
pair<NP, NP> merge2(NP a, NP b, F alz, F blz) {
  if (dep(a) < dep(b)) {
    ...
    return sub(x.first, x.second, push(br, blz));
  } else if (dep(a) > dep(b)) {
    alz = composition(alz, a->lz);
    NP al, ar; push_rev(a, al, ar);
    pair<NP, NP> x = merge2(ar, b, alz, blz);
    return sub(push(al, alz), x.first, x.second);
  } else { return {push(a, alz), push(b, blz)}; }
}
NP merge(NP a, NP b) {
  if (!a) return b;
  if (!b) return a;
...
pair<NP, NP> split(NP x, U s, F lz) {
  ...
  if (s <= size(xl)) {
    ...
    return {l, merge(r, push(xr, lz))};
  } else {
    auto [l, r] = split(xr, s-size(xl), lz);
    return {merge(push(xl, lz), l), r};
  }
}
vector<T> tovector(NP root) {
  ...
  while (!st.empty()) {
    auto [t, lz, nf] = st.top(); st.pop();
    if (t == nullptr) continue;
    if (t->size == 1) res.push_back(mapping(lz, t->v));
    else {
...
struct Tree {
  ...
  Tree(vector<T> &v) {
    auto dfs = [&](auto &&dfs, int l, int r) -> NP {
      ...
      int m = (l + r) / 2;
      NP L = dfs(dfs, l, m), R = dfs(dfs, m, r);
      return gen(L, R);
    };

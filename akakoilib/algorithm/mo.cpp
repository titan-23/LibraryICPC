struct Mo {
  using u64 = unsigned long long; u64 max_n;
  vector<pair<int, int>> lr;
  u64 order(u64 x, u64 y) {
    u64 res = 0;
  ...
  void run(const auto& al, const auto& ar, const auto& el, const auto& er, const auto& out) {
    int q = lr.size();
    vector<int> qi(q); iota(qi.begin(), qi.end(), 0);
    vector<u64> eval(q);
    rep(i, q) eval[i] = order(lr[i].first, lr[i].second);

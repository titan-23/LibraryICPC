template <class S, class T, T (*op)(T, T), T (*e)()>
class Seg2D {
private:
  S H, W; int root;
  struct NodeX { T data; int lch, rch; };
  struct NodeY { int root, uch, dch; };
  vector<NodeX> X; vector<NodeY> Y;
  ...
  void inner_setX(int node, S x, T v, S L, S R) {
    if (R-L == 1) { X[node].data = v; return; }
    S M = L+(R-L)/2;
    if (x < M) {
  ...
  void inner_setY(int node, S y, S x, T v, S U, S D) {
    ...
    if (D-U == 1) { inner_setX(Y[node].root, x, v, 0, W); return; }
    S M = U+(D-U)/2;
    if (y < M) {

...
struct LazyRBST {
  ...
  stack<NP> pL, pR; NP root;
  struct Node {
    NP left, right; T key, data; F lazy; bool rev; int size;
  ...
  pair<NP, NP> _split_node(NP node, int k) {
    while (node) {
      ...
      if (s <= 0) { pR.emplace(node); node = node->left; }
      else { k = s-1; pL.emplace(node);  node = node->right; }
    }
    NP l = nullptr, r = nullptr;
    while (!pL.empty()) {
      NP node = pL.top(); pL.pop();
      node->right = l; l = node; l->update();
    }
    while (!pR.empty()) {
      NP node = pR.top(); pR.pop();
...
public:
  ...
  void merge(RBST &o) { root = _merge_node(root, o.root); }
  ...
  T all_prod() { return this->root ? this->root->data : e(); }
  T prod(int l, int r) { if (l == r) return e();
    auto [a, bc] = split(l); auto [b, c] = bc.split(r - l);
    T res = b.all_prod();
    root = _merge_node(_merge_node(a.root, b.root), c.root);
    return res;
  ...
  void reverse(int l, int r) { if (l == r) return;
  ...
  void set(int k, T key) {
    NP node = root;
    while (1) { node->propagate(); pL.emplace(node);
      int t = node->left ? node->left->size : 0;
  ...
  int len() { return root ? root->size : 0; }
};
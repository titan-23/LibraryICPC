...
private:
  ...
  int new_node() {
    ...
    child.emplace_back(); suffix_link.emplace_back();
  ...
  int get_upper(int node, char c) const {
    while (node != 0) {
      if (n-len[node]-1 >= 0 && s[n-len[node]-1] == c) break;
...
public:
  EerTree() : n(0), all_count(0) {
    ...
    suffix_link[idx0] = 0; suffix_link[idx1] = idx0;
    suffix_link_dep[idx0] = 0; suffix_link_dep[idx1] = 0;
    par[idx0] = par[idx1] = 0;
    last_idx = idx1;
  }
  void add(char c) {
  ...
  int count_suffix_palindromes(int i) { return suffix_link_dep[suff[i]]; }
  ...
  // 空でないすべての回文の個数 / O(1)
  long long count_all_palindromes() { return all_count; }
  // 相異なる空でない回文の総数 / O(1)
  int count_unique_palindromes() { return (int)child.size()-2; }
  // 回文のidxと頻度配列 / O(?)
  vector<int> get_freq() {
  ...
  void print() {
    ...
    auto dfs = [&] (auto &&dfs, int node, int par) -> void {
      if (node > 1) res[node-2] = {par-1, suffix_link[node]-1};
      for (auto [_, c] : child[node]) dfs(dfs, c, node);
    };
